...
try {
  ...
  console.warn('emailService load error:', e.message);
}
...
router.post('/send-otp', async (req, res) => {
  ...
  if (!sendOTP) {
    return res.status(500).json({ error:'Email service failed to load on server. Please contact support.' });
  }
  try {
    ...
    return res.status(500).json({ error:`Could not send OTP: ${err.message}` });
  }
});
