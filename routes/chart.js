...
const { generateAISections } = require('../services/geminiReportService');
...
router.post('/report',
  ...
  async (req, res) => {
    ...
    try {
      ...
      // CREDITS TEMPORARILY DISABLED — re-enable by uncommenting below
      // if (credits < cost) return res.status(402).json({ error: `Insufficient credits.`, creditsRequired: cost, creditsAvailable: credits });
      ...
      // Build base report data (planetary positions, yogas, dashas etc)
      const reportData = buildReportData(chartData, userName, dob, birthTime, birthPlace, type);

      // Generate AI-powered personalised content via Gemini
      console.log('Starting Gemini AI report generation...');
      const aiSections = await generateAISections(
        chartData, userName, dob, birthTime, birthPlace,
        reportData.yogas, reportData.currentDasha, type
      );

      // Merge AI sections into reportData (AI overrides static text where available)
      if (aiSections.lagnaAnalysis)      reportData.lagnaInterp          = aiSections.lagnaAnalysis;
      if (aiSections.careerAnalysis)     reportData.careerPrediction      = aiSections.careerAnalysis;
      if (aiSections.financeAnalysis)    reportData.financePrediction     = aiSections.financeAnalysis;
      if (aiSections.marriageAnalysis)   reportData.marriagePrediction    = aiSections.marriageAnalysis;
      if (aiSections.healthAnalysis)     reportData.healthPrediction      = aiSections.healthAnalysis;
      if (aiSections.spiritualPath)      reportData.spiritualPath         = aiSections.spiritualPath;
      if (aiSections.remedies)           reportData.aiRemedies            = aiSections.remedies;
      if (aiSections.personalitySummary) reportData.lifeSummary           = aiSections.personalitySummary;
      ...
      const updated = { rows: [{ credits: credits }] }; // no deduction
...
function buildReportData(chart, userName, dob, birthTime, birthPlace, type) {
  ...
  const planetTable = Object.entries(planets).map(([name, data]) => ({
    ...
    sign: data.sign || '—',
    degrees: data.degrees ?? null,                           // raw number for PDF
    degreesFormatted: data.degrees ? `${data.degrees.toFixed(2)}°` : '—',
    house: data.house || '—',
  ...
  const data = {
    ...
    ascSignIdx: ascendant?.signIndex ?? 0,
