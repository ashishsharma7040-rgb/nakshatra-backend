...
router.post('/ask',
  ...
  async (req, res) => {
    ...
    // 2. Load chart — now includes dob, birth_time, birth_place for personal context
    let chartData, dob, birthTime, birthPlace;
    try {
      const chartRow = await pool.query(
        'SELECT chart_data, dob, birth_time, birth_place FROM charts WHERE user_id=$1',
        [userId]
      );
      ...
      dob        = chartRow.rows[0].dob;
      birthTime  = chartRow.rows[0].birth_time;
      birthPlace = chartRow.rows[0].birth_place;
    } catch (err) {
      return res.status(500).json({ error: `Chart load error: ${err.message}` });
    }

    // 3. Call Gemini with full personal context
    let answer;
    try {
      answer = await getAIReading(chartData, question, userName, dob, birthTime, birthPlace);
    } catch (aiErr) {
