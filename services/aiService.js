      ...
      res.on('end', () => {
        try {
          ...
          if (res.statusCode === 200) {
            ...
            err.isQuota = res.statusCode === 429 || msg.includes('quota') || msg.includes('429');
            reject(err);
          }
...
async function getAIReading(chart, question, userName) {
  ...
  // Updated March 2026 — gemini-2.5-flash is the current free tier model
  const models = [
  ...
  throw new Error('The stars are busy right now. Please try again in a minute.');
}
